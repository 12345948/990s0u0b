...
let proxyIPsapi = []; // 新增 PROXYIP API
let 匹配PROXYIP = [];
let 匹配PROXYIPapi = []; // 新增 CMPROXYIPS API
let socks5DataURL = '';
let BotToken = '';
let ChatID = '';
let 临时中转域名 = [];
...
export default {
	async fetch(request, env) {
		...
		if (env.CMPROXYIPSAPI) 匹配PROXYIPapi = await 整理(env.CMPROXYIPSAPI); // 新增 CMPROXYIPS API
		...
		if (env.PROXYIPAPI) proxyIPsapi = await 整理(env.PROXYIPAPI); // 新增 PROXYIP API

		// 处理 PROXYIP API
		if (proxyIPsapi && proxyIPsapi.length > 0) {
			const newProxyIPs = await 整理优选列表(proxyIPsapi);
			proxyIPs = Array.from(new Set(proxyIPs.concat(newProxyIPs).filter(item => item && item.trim())));
		}

		// 处理 CMPROXYIPS API
		if (匹配PROXYIPapi && 匹配PROXYIPapi.length > 0) {
			const new匹配PROXYIP = await 整理优选列表(匹配PROXYIPapi);
			匹配PROXYIP = Array.from(new Set(匹配PROXYIP.concat(new匹配PROXYIP).filter(item => item && item.trim())));
		}
